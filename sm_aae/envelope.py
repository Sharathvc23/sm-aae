...
"""Signed pre-action authorization envelopes forming a per-agent hash chain.
...
envelope exactly like a grant, so "we said no" is provable later, not merely
an absence of evidence.

An envelope has exactly eight fields:

- ``agent_id`` — whatever identity string the caller's system yields.
...
- ``issued_at`` — RFC 3339 timestamp, **always caller-supplied**; validated
...
"""An authorization envelope document (plain dict; see the module docstring)."""
