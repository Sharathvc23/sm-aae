...
sm-aae was correct at the time of this change. That is the point: correct-today
...
def test_dunder_version_equals_installed_distribution_version() -> None:
    assert sm_aae.__version__ == dist_version("sm-aae")
...
def test_dunder_version_is_not_the_uninstalled_sentinel() -> None:
    ...
    assert sm_aae.__version__ != "0.0.0.dev0"
...
def test_module_source_declares_no_version_literal() -> None:
    ...
    correct.
    """
    src = Path(sm_aae.__file__).read_text(encoding="utf-8")
...
def test_pyproject_remains_the_single_declared_source() -> None:
    ...
    assert declared == dist_version("sm-aae"), (
